...
function builddssanxuat(page,record) {
    ...
    $(".listnhacungcap").html("<img src='img/loading.gif' width='20px' height='20px'/>");
...
$(".numberpagenhacc").on('click','button',function () {
    ...
    builddssanxuat($(this).val(),record);
...
function resetViewncc() {
   ...
   $(".ncc_lat").val("0");
    ...
    $(".titlencc").html("<i class='fa fa-plus'></i>&nbsp; Tạo mới Hộ sản xuất - Nhà cung cấp");
...
function datasendncc(){
	...
	var latnew= $(".ncc_lat").val();
    var lngnew= $(".ncc_lng").val();
    var dataSend={
        username:u,
        password:p,
        permission:1,
            ...
            queryData("nhacungcap/create", dataSend, function (res) {
               // console.log(res);
                if (res.code == 1000) {
                    ...
                    builddssanxuat(ncc_current,record);
                    resetViewncc();
                    //getsanphamData();
...
$(".listnhacungcap").on('click',".btn-show-info-suancc",function () {
 ...
 $(".ncc_lat").val(custom.lat);
  $(".ncc_lng").val(custom.lng);
...
$(".listnhacungcap").on('click',".btn-show-info-xoancc",function () {
    ...
    bootbox.confirm("Bạn có chắc xóa hộ sản xuất "+id+" này không?", function(result){
        if(result==true) {
           ...
           // console.log(dataSend);
            ...
            queryData("nhacungcap/deletenhacungcap", dataSend, function (res) {
                ...
                if (res.code == 1000) {
                    ...
                    alert_info("Hộ sản xuất này không thể xóa. Do đã đang sử dụng");
                    $(".modalshowprogess").modal("hide");
                }else{
				 alert_info("Lỗi lấy dữ liệu");
				}
				$(".modalshowprogess").modal("hide");
            });
...
$(".modalshowncc").on("click",".btnmap_addressncc",function () {
    $(".ncc_addressncc").val('');
    $("#complete").val('');
