...
var o=JSON.parse(localStorage.getItem("userVINATT"));
     ...
     console.log(res);
        if(res.code==1000){
			 ...
			 for (item in data) {
			  ...
			  //namework=list.namebatch;
			  datework=list.createdate;
		//	html='<div portlet-content table-responsive>IDSP:'+list.qrcode+'<br> Ngày tạo công việc:'+getDateTime(list.createdate)+'<br> Tên công việc:'+list.namew+'</div>';
			
			// $(".list_works_famers").append(html);
			//	updateImageLinkWorkpreview(html,item,list.qrcode,list.createdate,listhinh,"list_works_famers","imguserchange_anhworkpre","removelinkworkpre","savelinkworkpre");
