...
var snd="",ssoche="",snhavc="",snhapp="";
 ...
 }else if(role==2)//nha so che
 {
    ...
    queryData("dangkysp/checkuservc",dataSend,function (res) {
         if( res.code==1003){
			 alert_info("User này không tồn tại trong hệ thống");
 ...
 var name="",bienso="",phone="";
 $(".btn_ptvc").click(function () {
	
	 name= $(".pt_nametx").val();
	  phone= $(".pt_phonetx").val();
	 bienso= $(".pt_bienso").val();
	 // alert(name+phone+bienso);
	 if(name!=""&& phone!=""&&bienso!=""){
		 
	 updatevc(idsp,iduser,name,phone,bienso);
	 }else{
		alert("Điền thông tin đầy đủ");
	 }
 });
 
 $(".modalshowinfornvc").on("hidden.bs.modal", function () {
    // put your default event here
	if(phone=="" && bienso=="" && name==""){
		$(".modalshowinfornvc").modal("show");
	}
});
...
function updatevc(idsp,iduser,name,phone,bienso) {
